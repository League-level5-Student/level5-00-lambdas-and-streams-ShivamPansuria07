...
public class Minesweeper extends PApplet {
	...
	void revealAllCells() {
		...
		// Stream<Cell> streamCell = cells.stream();

		cells.stream().forEach((i) -> i.flagged = true);
	 ...
	 */
	boolean checkWin() {
		if (cells.stream().filter((i) -> i.mine == false).filter((i) -> i.revealed == false).count() > 0) {
			return false;
		} else {
			// this took forever
	...
	void setNumberOfSurroundingMines() {
		cells.stream().forEach((i) -> i.minesAround = getNeighbors(i).stream().map((y) -> y.mine ? 1 : 0).reduce(0,
				(acc, next) -> acc + next));
	}
